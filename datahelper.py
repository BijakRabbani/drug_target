...
def parse_pssm(file):
    #with open(prot_id + '.txt', 'r') as file:
    file.readline()
    file.readline()
    ...
    index = result[1]
    result.drop([0,1, 42, 43],1, inplace = True)
    result.index = index
...
class DataSet(object):
  ...
  def parse_data_fingerprint(self, fpath,  with_label=True): 
		
    print("Read %s start" % fpath)

    ligands = pd.read_csv(fpath+'fingerprint.csv').T.astype(np.float32).values
    proteins = json.load(open(fpath+"proteins.txt"), object_pairs_hook=OrderedDict)
  ...
  def parse_data_pssm(self, fpath,  with_label=True): 
